...
ADMIN_ID = int(os.getenv("ADMIN_ID", "1091526567"))

# مسار قاعدة البيانات - آمن لا يُحذف عند إعادة النشر
DB_PATH = os.getenv("DB_PATH", "/app/data/proxy_bot.db")
...
def add_proxy_request(user_id, proxy_type, plan_name, cost):
    conn = get_conn()
    c = conn.cursor()
    c.execute("INSERT INTO proxy_requests (user_id, proxy_type, plan_name, cost) VALUES (?, ?, ?, ?)",
              (user_id, proxy_type, plan_name, cost))
    conn.commit()
    conn.close()
...
async def send_order_notification(context, user_id, first_name, proxy_type, plan_name, cost, remaining_balance):
    """يرسل الإشعار للمجموعة - يعمل مع Forum والعادية"""
    order_text = (
        f"📢 طلب بروكسي جديد!\n\n"
    ...
    # محاولة 1: إنشاء Topic (إذا كانت المجموعة Forum)
    try:
        ...
        print(f"Forum topic failed: {e}")

    # محاولة 2: إرسال عادي (إذا كانت المجموعة عادية)
    try:
        await context.bot.send_message(
            chat_id=ORDERS_GROUP_ID,
            text=order_text
        ...
        print(f"Order notification failed: {e}")
        return None
...
def proxy_plans_keyboard(proxy_type):
    ...
    for key, plan in PROXY_PLANS.items():
        keyboard.append([InlineKeyboardButton(f"{plan['name']} - {plan['cost']} نقطة",
                                              callback_data=f"pplan_{proxy_type}_{key}")])
...
async def buy_ultra_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_order_notification(
        context, user_id, query.from_user.first_name,
        "⚡ ألترا سوكس", "جلسة اقتصادية", ULTRA_COST, get_balance(user_id)
    )
...
async def proxy_plan_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_order_notification(
        context, user_id, query.from_user.first_name,
        ptype['name'], plan['name'], plan['cost'], get_balance(user_id)
    )
...
async def admin_backup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not os.path.exists(DB_PATH):
        await update.message.reply_text("⚠️ قاعدة البيانات غير موجودة بعد.")
        return
        ...
        with open(backup_path, 'rb') as f:
            await update.message.reply_document(
                ...
                caption=f"📦 نسخة احتياطية من قاعدة البيانات\n\n"
                        ...
                        f"💾 الحجم: {os.path.getsize(backup_path) / 1024:.2f} KB\n\n"
                        f"📌 احفظ هذا الملف في مكان آمن."
            )

        os.remove(backup_path)
    except Exception as e:
        await update.message.reply_text(f"⚠️ فشل إنشاء النسخة: {e}")


async def admin_restore(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """استعادة قاعدة البيانات من ملف مرسل"""
    if update.effective_user.id != ADMIN_ID:
        return
    ...
    if not update.message.reply_to_message or not update.message.reply_to_message.document:
        await update.message.reply_text(
            "⚠️ الاستخدام:\n"
            "1. أرسل ملف قاعدة البيانات (backup.db)\n"
            "2. قم بالرد عليه بالأمر /restore"
        )
        return
    ...
    try:
        ...
        if not doc.file_name.endswith('.db'):
            await update.message.reply_text("⚠️ الملف يجب أن يكون بصيغة .db")
            return
        ...
        await update.message.reply_text(
            ...
            "📌 يُنصح بإعادة تشغيل البوت للتأكد من التحديث."
        )
    except Exception as e:
        await update.message.reply_text(f"⚠️ فشل الاستعادة: {e}")
...
async def admin_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    c.execute("SELECT COUNT(*) FROM proxy_requests")
    total_requests = c.fetchone()[0]

    c.execute("SELECT COUNT(*) FROM free_proxies WHERE used = 0")
...
def main():
    if not TOKEN:
        print("❌ خطأ: لم يتم تعريف BOT_TOKEN في متغيرات البيئة!")
        return
